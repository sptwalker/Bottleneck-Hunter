...
class _SimTradingMixin:
    ...
    def set_default_vip_account(self, account_ref: str = "") -> dict:
        ...
        with self._write_conn() as conn:
            ...
            conn.execute(q, p)
            q, p = self._filtered(
                "UPDATE vip_accounts SET is_default=1, updated_at=? WHERE account_ref=?", (_now_iso(), account_ref)
            )
            conn.execute(q, p)
        ...
        try:
            ...
            conn.execute(
                f"""INSERT INTO sim_account
                   ...
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?{self._user_insert_vals()}{self._market_insert_vals()})""",
                (aid, name, capital, capital, capital, capital, 0.0, 0, 0.0, target_ref, now, now)
                + self._user_insert_params()
                + self._market_insert_params(),
            )
            conn.commit()
    ...
    def update_sim_account(self, account_ref: str = "", **fields) -> bool:
        ...
        with self._write_conn() as conn:
            q, p = self._filtered(f"UPDATE sim_account SET {', '.join(parts)} WHERE id = ?", tuple(vals))
    ...
    def get_sim_positions(self, account_id: str | None = None, include_zero: bool = False) -> list[dict]:
        ...
        try:
            ...
            if account_id:
                q, p = self._filtered(f"SELECT * FROM sim_positions WHERE account_id = ?{share_filter}", (account_id,))
            ...
            conn.execute(
                f"""INSERT INTO sim_trades
                   (id, account_id, execution_plan_id, entry_id, ticker, side,
                    shares, price, amount, trade_type, reasoning, slippage_bps, realized_pnl, created_at
    ...
    def delete_sim_position_if_zero(self, position_id: str) -> bool:
        ...
        with self._write_conn() as conn:
            q, p = self._filtered("DELETE FROM sim_positions WHERE id = ? AND shares = 0", (position_id,))
            cur = conn.execute(q, p)
            return cur.rowcount > 0

    def create_sim_position(
        self, account_id: str, ticker: str, shares: int, avg_cost: float, entry_id: str | None = None
    ) -> str:
        ...
        with self._write_conn() as conn:
            conn.execute(
                f"""INSERT INTO sim_positions
                   ...
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?{self._user_insert_vals()}{self._market_insert_vals()})""",
                (pid, account_id, entry_id, ticker, shares, avg_cost, avg_cost, shares * avg_cost, 0.0, 0.0, now, now)
                + self._user_insert_params()
                + self._market_insert_params(),
            )
            return pid
    ...
    def update_sim_position(self, position_id: str, **fields) -> bool:
        allowed = {"shares", "avg_cost", "current_price", "market_value", "unrealized_pnl", "weight_pct", "entry_id"}
        parts, vals = [], []
        for k, v in fields.items():
            if k in allowed:
                parts.append(f"{k} = ?")
        ...
        with self._write_conn() as conn:
            q, p = self._filtered(f"UPDATE sim_positions SET {', '.join(parts)} WHERE id = ?", tuple(vals))
            cur = conn.execute(q, p)
            return cur.rowcount > 0
    ...
    def create_fund_op(self, account_id: str, op_type: str, amount: float, note: str = "") -> str:
        ...
        with self._write_conn() as conn:
            conn.execute(
                f"""INSERT INTO sim_fund_ops
                   ...
                   VALUES (?,?,?,?,?,?{self._user_insert_vals()}{self._market_insert_vals()})""",
                (fid, account_id, op_type, amount, note, _now_iso())
                + self._user_insert_params()
                + self._market_insert_params(),
            )
            return fid
    ...
    def get_fund_ops(self, limit: int = 20) -> list[dict]:
        conn = self._connect()
        try:
            q, p = self._filtered("SELECT * FROM sim_fund_ops ORDER BY created_at DESC LIMIT ?", (limit,))
            rows = conn.execute(q, p).fetchall()
            return [dict(r) for r in rows]
        finally:
            conn.close()
        ...
        try:
            rows = [
                ...
                for r in conn.execute(
                    "SELECT * FROM mail_ingest_log ORDER BY created_at DESC LIMIT ?", (limit,)
                ).fetchall()
            ]
        finally:
            conn.close()
