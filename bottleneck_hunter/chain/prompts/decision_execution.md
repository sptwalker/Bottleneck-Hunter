...
### 硬性约束（必须满足——违反的操作会被系统直接拦截、无法执行）

以下是当前 regime 与风险偏好下**实际生效**的约束（已按市场状态收紧，不是通用默认值）。你生成的每一条操作都必须落在这些上限之内，否则会被拦截：

{constraints}

> 生成前请逐条自检：买入后单股占比、板块占比、现金比例、单笔与单日规模是否都在上限内。若某标的加满后仍超限，请**主动缩减股数**到合规，而不是输出超限计划让系统拦截。宁可少买，不可超限。
...
**语言要求：所有文本字段（rationale / reason / contingency 等）必须用简体中文，不得使用英文。**
...
- **但"该配没配"同样是一种偏差**：若组合实际权益显著低于 Layer 2 目标下限（或现金比例显著高于目标上限）而账上仍有闲置现金，**"不部署"和"超限买入"一样需要理由**。此时请优先按 Layer 2 的目标权重，对**低配的核心持仓**生成加仓/建仓操作，把资金配到目标结构上；若确有理由不下单（如数据过期、催化剂临近但方向不明），请在 `skipped_plans` 里写明原因，不要静默略过。
- 买卖对称：你既负责控住上限（不超限），也负责够到目标下限（钱别趴着）。
