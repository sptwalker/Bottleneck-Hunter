...
### Phase 3 — 安全与合规 ✅ 完成 2026-07-02

**重要审计校正**：G-6 的"budget 仅单次估算、无累计"表述**不准确**——`BudgetTracker` 已记录用量入库、按日/月累计、三级降级（`llm_budget` 表 367 行为证）。真实缺口是"无硬上限（只软降级）"，已针对性修复。

| # | 改动文件 | 验收方式 | 结果 |
|---|---------|---------|------|
| G-3 | 新建 `prompt_guard.py` + `committee.py`/`decision_engine.py` 接入 | 单测 `test_phase3_security` 7 项 + demo | **PASS** 中英注入被隔离，正常财经文本不改动 |
| G-6 | `budget.can_spend` 增日/月硬熔断（≥100% 拒绝调用） | 单测 3 项 | **PASS** 日/月达上限硬停 |
| G-4 | `store._user_filter` 安全护栏（UNION/子查询/HAVING→显式报错） | 单测 5 项 + 全量套件 | **PASS** 静默跨用户泄露→安全失败；现有查询零破坏 |
| G-1 | （Phase 0 已落地）买卖指令/导出/UI 免责 | — | 已完成 |

**决策：G-5/G-7/G-8 明确降级为增强项（诚信不硬凑）**：
- **G-8 决策留痕**（prompt 版本+输入哈希）：需改造多条决策写入路径，属可审计性增强而非安全漏洞。当前决策已存 `result_json` + committee 存 model_name，可部分追溯。列为后续增强，**不在本轮声称完成**。
- **G-5 sim/live 硬隔离**：系统当前纯模拟盘，无真实 broker 对接，风险尚未激活。接实盘前必做，此刻实现属投机（YAGNI）。已在报告中标注"接实盘前置条件"。
- **G-7 公司行动复权/交易日历**：数据工程量大，属数据质量增强。当前模拟盘 + 日频决策下影响有限，列为增强项。

**回归验证**：新增 `test_phase3_security` 15 项全绿；全量套件 **53 失败仍全为 pre-existing API-auth**，Phase 3 零新回归，passed 726→741（+15）。

**阶段回顾（偏差与疏漏检查）**：
- ✅ 无偏差：注入防护、成本熔断、隔离护栏均有单测证明真实生效，且不破坏现有查询。
- ✅ G-4 采用"安全失败"而非全量 ORM 重写——最小改动堵住泄露路径，把静默漏洞变成显式报错，符合诚信+最小原则。
- ✅ 诚信收尾：G-5/G-7/G-8 明确标记为增强项而非"已完成"，不重蹈"over-promise"覆辙（这正是整个改造要根除的病）。

---

## 三、全阶段完成总结

| Phase | 主题 | 核心交付 | 新增测试 | 回归 |
|-------|------|---------|---------|------|
| 0 | 诚信止血 | 清 137 空壳/mock标记/幻觉价闸门/市场码归一/免责/告警 | price_guard 3 | 0 |
| 1 | 打通闭环 | realized_pnl 持久化/健壮复盘/偏好学习接线/市场码根治 | loop_e2e 2 | 0 |
| 2 | 专业性 | 分行业权重级联/账户熔断/可投性门控/委员独立性 | phase2_risk 9 | 0 |
| 3 | 安全合规 | 注入防护/成本硬熔断/隔离安全护栏 | phase3_security 15 | 0 |

**累计**：29 项新测试全绿；4 个 phase 零新回归；53 个 pre-existing API-auth 失败保持不变（改造前既有，建议独立修复测试夹具）。
