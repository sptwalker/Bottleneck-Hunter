...
2. **`existing_n` 语义confusion**：`get_sim_positions` 默认已滤 `shares > 0`，故 `existing_n` 是**活仓**数（墓碑不计）。这正是零持仓纯衍生品账户 `existing_n` 恒为 0 的原因——即 N-34 骤降分支失效的根因。已在函数注释与 P1-K 实现处写明，防后人误判。
