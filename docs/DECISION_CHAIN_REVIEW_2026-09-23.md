...
> **生产基线**：`main@72bc42f`，容器 `bottleneck-hunter` 已 grep 证真扩张侧代码全部上线。
...
| 账户记账一致性 | **①头条权益/现金已于 2026-09-24 经授权改回 07-31 期真值（改前库备份 `data/watchlist.db.bak-cmbis-equityfix-20260924T020356Z`）；②③待代码修复（P1-L / P0-F）**。**CMBIS 一个账户、三处期次错配**（一次乱序上传造成的连锁）：① 头条权益与现金停在 **04-30** 期（真值应取 07-31，`1,065,337.14`→`1,063,096.50`、`8,037.40`→`16,028.50`）—N-34；② 衍生品当期 MTM 误取 **07-07 的 NVDA + 04-30 的两笔已到期 CMBIGP**，显示 `2,099,173.74` 而真值 `1,047,068.00`，**高估约 2.0×**—N-35；③ 排序键 `vip_derivative_terms.created_at` **裸写北京本地时间**、与全库 UTC 口径差 8 小时—N-36。根因＝零股票持仓账户的两道导入护栏同时失效，**净值与持仓明细均由「上传顺序」而非「结单期次」决定**（N-34/N-35 均已铁证闭环；**"零持仓→positions 零行"已查证为真实链路行为，非探针 artifact**）。另有 25 票「墓碑+活行」并存（CITI-1 18 / NOMURA 7），**全在 VIP 账户、决策中心自有账户零命中、当前不可达**（N-33） |
...
**N-33【中高·已本地复现·当前不可达】`sim_positions` 同一账户同一标的存在「墓碑行 + 活行」时，`get_sim_position_any` 可能命中墓碑，导致买入写错行 → 组合市值重复计数、减仓卖错行。**
...
**N-35【中高 · 生产实测 · 新发现】衍生品条款按「上传时间」而非「结单期次」选当期 → CMBIS 对外展示的 FCN 持仓明细与 MTM 是**过期期次**（07-07 与 04-30），且包含一笔**已于 2026-05-15 到期**的产品。**
...
**N-36【中 · 工程 · 新发现】`vip_derivative_terms.created_at` 用**北京本地时间裸写**，违反项目 UTC 存储约定；且它正是 N-34 修复所依赖的排序键。**
...
- **改法（二选一，建议 (b)）**：
  ...
  - 同时：**到期日已过（`lot_key` 尾部的 `:YYYY-MM-DD` < 今天）的条款，不应作为"当期条款"并入持仓构成**。建议在 `_current_derivative_rows` 出口按此过滤（或在 `_derivative_holdings` 内），并**留痕说明剔除了几笔**，不静默。
...
- **P1-K + P1-L + P0-F**：账户记账域的三件套（N-34 净值期次错配 / N-35 衍生品期次错配 / N-36 排序键时区）。**已有生产实证**（CMBIS 头条权益按 04-30 计、衍生品 MTM 高估约 2.0×），且是后续一切账户对账的地基——排在第 0 批。**三者同批**：P0-F 修排序键时区，是 P1-K/P1-L 的判据前提；P1-L 与 P1-K 在同一个函数窗口内，分两次改等于把同一段逻辑摸两遍。
...
- **P2-A / P2-B** 打磨；**P2-I**（墓碑行歧义，N-33）——**前置条件尚未满足**：需先定调"VIP 账户将来是否会被下单"，不定调就先不动代码。
...
> **本文为只读复核 + 方案，未改动任何代码。** 所有改动**待用户单独授权**再动手。
