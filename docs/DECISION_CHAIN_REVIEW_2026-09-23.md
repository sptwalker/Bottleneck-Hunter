...
> ⚠️ **上句"本次未动"仅指第 0 批这一次提交**。其余方案此后已由 Batch A/B/C 全部处理完毕，见 §七/§八/§九；总览与仅存的三项刻意未做项见**文末"落地总览"**。
...
- **生产部署与生产证真**：**已随 `602926a` 上线**（2026-09-26；祖先关系已核：本批提交 `846e065`/`368ff87` 均为 `602926a` 的祖先，生产容器运行的就是 `602926a`）。**本节未单独留档上线后证真** —— `602926a` 那次容器内逐处证真覆盖的是该提交全量 diff，其中含本批改动，但未按批拆分记录。此行的原话"尚未提交、尚未上线"系提交前的快照，已于 2026-09-26 更正。
...
- **生产部署与生产证真**：**已随 `602926a` 上线**（2026-09-26；祖先关系已核：本批提交 `6599eae` 为 `602926a` 的祖先）。**本节未单独留档上线后证真**，口径同 7.6。此行的原话"尚未提交、尚未上线"系提交前的快照，已于 2026-09-26 更正。
...
- **`daily_commentary` 的消费方**：见 9.6(b)，需先定调"持久化还是点进前端"，本批只记账（用户 2026-09-26 指示"建议不做的都先不做"，维持）。
- **P1-A 后半（`result_json` 保留 `gap_driven`/`opportunity_driven` 标记）**：核验后**建议不做**，用户已同意。生产实测近 60 条执行计划里两个标记**各 0 条**，但 58/60 带 `tactical_plan_id`、经其回查即可确认驱动归属（实测可追出 1 条）。加冗余标记是给冗余付费——除非将来审计查询要省掉这次 join。
...
- **生产部署与证真**：**已于 2026-09-26 完成**（提交 `602926a` → `./deploy.sh` 重建镜像 → 容器内 `inspect.getsource`/`grep` 逐处证真），见 **9.11**。本节此后无新增未做项；**本批全部收尾工作已于同日完成并随 `03e6e2a` 再次上线**（见文末"落地总览"）。

### 9.11 核验时发现的第三处漏记：P1-F / P1-G 已上线但零记录、零护栏（2026-09-26 补记）

**发现经过**：复核"还有无剩余未完成任务"时按 diff 反向核对，发现 `P1-F（N-12）` 与 `P1-G（N-14）` 的代码由 **`602926a`（即本批提交本身）引入并在生产生效**，但 §七/§八/§九 三个执行记录节**一处都没写它们**——`docs/` 全文搜 `P1-F`/`P1-G` 只有第四节的两条**修复方案原文**。

这是**同一类漏记的第三次发作**（前两次：P2-D → §8.6，N-11 → §8.7）。前两次是"工作树里有、范围表没写"；这次更进一步：**它们是我自己在本次提交里带进仓的，却连自己的执行记录都没进**。故本节的结论比前两次更硬：**范围表与执行记录都不等于已完成清单，提交前必须按 `git show --stat` + 逐 hunk 反向核对**，只读差异清单不够。

**更实质的是护栏**。两项功能逻辑逐行读过、都正确，但报告给它们各自写的**验证条款都没落地**，且全量 2374 条里**没有一条**会因它们被改坏而变红。变异实测（写护栏时逐轮重跑，含一次自我更正）：

| 变异 | 复现的修复前语义 | 补测前 | 补测后 |
|---|---|---|---|
| **P1-F** | 逃逸候选集退回 `(权益, 现金)` 两项，板块漂移不进判据 | **0 击杀**（`-k "drift or sector or clamp"` 42 条全过） | **1 击杀**（`test_板块漂移单独触发逃逸`） |
| **P1-G** | 删掉类型修复 + 告警块，退回"静默跳过" | **0 击杀**（`-k "clamp or discipline or target_allocation"` 33 条全过） | **3 击杀**（类型修正 / 类型不可用 / 端到端污染三条） |

> 全仓亦无 `板块[...]` 逃逸的用例（`grep -rn "板块\[" tests/*.py` 空）——报告 P1-F 写的验证条款（"权益/现金都贴目标但某板块漂移 > 15 → 断言逃逸"）从未被实现。

**写护栏时踩到并修掉的一个自身缺陷**（记在本节，因为它与本节主题是同一类病：*断言写得漂亮但不成立*）：

P1-F 的第一版夹具用**两个**板块（半导体目标 40%、软件 20%），实际给 10% / 50%。首轮全量跑出来是红的，失败信息为 `板块[软件]偏离目标 +30.0pct`，而断言要的是 `半导体`。**这不是被测代码错，是我的用例错**：两板块时板块权重互补（`a + b = 权益`）、目标也互补，于是两边偏离**恒等值反号**（semi 10−40 = −30 / soft 50−20 = +30），`max(..., key=abs)` 在并列时退化成"取列表首个"，而候选列表来自 `set(...)` 迭代 —— 跨进程随 hash seed 变序。实测同一用例连跑 12 次：**5 passed / 7 failed**（`PYTHONHASHSEED` 每进程不同）。

修法是**让最大值唯一**：改成三个板块（目标 40/10/10，实际 20/20/20 → 半导体 −20 唯一越线，另两个各 +10 都在带内），连跑 15 次全绿且确定。

> **这条比 P1-F 本身更值得记**：若只按"变异击杀数"验收，第一版夹具在变异下**同样会红**（A 变异 1 击杀、B 变异也红），看上去完全合格 —— **唯一暴露它的是全量门禁 + 逐条查因**。一个用 `set` 迭代序做并列裁决的断言本身就是掷骰子；而"掷骰子掷出了红灯"这件事，只有在你要求自己解释为什么红的时候才会现形，光看击杀数永远是绿数。

**同轮全量门禁还捞出一条与本批无关的既存真 bug**（`test_yf_gate.py::test_throttle_truncates_wait_at_budget` 稳定失败 8/8）：

`throttle()` 里把 `_next_at` 截到 `now + left` 之后，`wait` 恰好等于 `(now+left)-now`，即 `left` ± 浮点噪声（实测 `wait-left = -2.3e-11`），紧接着的严格 `wait >= left` 便成了**抛硬币**：丢了硬币就照睡满 `_interval`，而这正是该层注释里明写要防的"300s 预算跑成 327s"。因变量只是 `time.monotonic()` 的量级 —— 它决定 `wait-left` 的符号（在 `now ≈ 1e3 / 1.5e5 / 2e6` 时为正、`5e4 / 2.7e5~1e6 / 1e7` 时为负），故这**不是回归，是一颗按开机时长随机亮的雷**；上一批全量绿只是那时网格位置恰好站对了边。

修法是一行**删除**：那行截断本身冗余 —— 守卫一旦正确，截断永远无效（`wait` 不是 `< left` 就是 `≈ left`，两者都落不进"`_next_at` 已被截"的情形）。删掉后直接拿真实槽位等待与余量比，量级差 2~3 个数量级，无刀锋。三场景实测：预算 0.1s/槽 30s → **0.000s 秒抛**；预算 100s/槽 0.3s → 正常睡 0.297s；"预算 = 槽位"边界连跑 6 次稳定收敛到"抛"（保守侧）。变异有效性：把那行加回去 → 击杀 1 条。

**补上的护栏**：`tests/test_p1f_p1g_guards.py`（6 条，本批新增）。刻意各带一条**变异复现用例**，使"是否真的接上了"这件事本身可执行：

- **P1-F 三条**：① 权益/现金精确贴 60/40、仅半导体 20% vs 目标 40% → **必须逃逸**，且断言 `equity_drift_pct`/`cash_drift_pct` 本身在阈值内（**反证判据确实来自板块**，而不是被权益/现金顺带触发）；② 板块全在带内 → 不逃逸（防"有明细就无脑逃逸"，那会每轮烧一次 L2）；③ 把 `sector_drift` 摘掉后**必须逃逸不了**——这条是①的非恒真证明。
- **P1-G 三条**：① `"55%"` → 修成 `55.0` 且有 `类型修正` 留痕；② 不可救类型（`list`）→ 有 `类型不可用` 留痕**且 `equity_pct` 填回 `recommended_equity`**（钉住 N-14 的真正病根：留 `None` 会让 `has_target` 塌掉 → `rebalance_suggested` 变 `None` → 逃逸机制**永久沉默且无痕**）；③ 端到端：L2 目标带字符串畸变时，逃逸判据**仍必须能工作**。

**顺带核实的两项（结论：无需动作）**：

- **`rebalance_suggested`（N-12 后半，报告称"零消费者"）**：**报告已过时**。它在 `decision_engine.py:1390-1393` 有真实消费者——`run_deviation_check` 用它决定 `rebalance_needed`（`None` 时退回 LLM、否则取"LLM 判定 or 确定性判定"）。该接线由 `4c2f348` 引入，早于本批。
- **P1-H（N-30）**：已落地。调用点 `decision_engine.py:1846-1854` 有 `try/except BLE001`，且注释逐条列了实探过的崩法（字符串权重 `"12%"` → `ValueError`、`core_holdings` 非 `list[dict]` → `AttributeError`、脏数值 → `TypeError`），口径与同级的放大器一致。

**门禁（本批提交前，冻结工作树实跑）**：全量 **2380 passed / 5 skipped / 0 failed**（407.07s，exit 0；`--collect-only` **2385** = 2380 + 5，较上批 2379 恰增新文件的 6 条）。新增 6 条全绿（连跑 15 次无摆动）；`tests/test_p1f_p1g_guards.py` + 既有两个逃逸/钳制测试文件共 **32 passed**；`tests/test_yf_gate.py` 修复后连跑 10 次 **8 passed**，且模块自检 `python -m ...yf_gate` 通过；`ruff check` 触达文件**零新增告警**（新夹具首版曾引入 1 条 B905，已改掉）；变异已全部还原，`git diff --stat HEAD -- bottleneck_hunter/` 仅剩本轮修复本身。
...
> **本文的复核与方案部分为只读产物，未改动任何代码。** 第四节之后的**执行记录**（第六~九节）是本文档唯一的"已完成"口径，以它为准，不要以第四节的方案清单为准。

**落地总览（2026-09-26 更新；四批全部完成后重写，此前这里的"仍待逐项确认"已过期）**

| 批次 | 授权 | 范围 | 执行记录 |
|---|---|---|---|
| **第 0 批** | 2026-09-24「VIP 账户未来不会下单」+「批准所有修复」 | P0-F / P1-K / P1-L / P2-I | **§六** |
