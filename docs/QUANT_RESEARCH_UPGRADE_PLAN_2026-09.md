...
| P0-4 | PIT 可见性、泄漏检测、缺失/降级审计 | ⬜ | P0-1/P0-2 | 新增验证模块及 pipeline 接入 | 人工构造未来数据、修订、缺失、降级场景 | 不可见观测被拒绝；缺失不静默填充 | 关闭门禁仅限离线迁移，不得生产绕过 |
| P0-5 | 事件驱动回测核心 | ⬜ | P0-1/P0-4 | 新回测模块、交易日历、执行模型 | 事件顺序、停牌、退市、现金、成本、滑点 | 与模拟成交回放明确分层；结果可复现 | 保留现有模拟盘回放入口 |
...
## 当前状态
P0-0 已完成；其余子项未开始。现有风险指标、决策闭环、真实快照成交、数据源降级和多用户/市场隔离可复用；P0 的完整快照、历史可见性、事件回测与严格样本外评估仍需补齐。已有工作区修改和未跟踪文件不属于本方案，实施时不得覆盖、删除或擅自提交。
